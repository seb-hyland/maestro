...
pub(crate) fn run_shellcheck(input: &str) -> Result<(), String> {
    let mut child = Command::new("shellcheck")
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| format!("Failed to spawn script checker!\n{e}"))?;
    ...
    let output = child
        .wait_with_output()
        .map_err(|e| format!("Failed to wait on script checker!\n{e}"))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(format!("The script has errors!\n{stderr}"))
    } else {
        Ok(())
    }
}
